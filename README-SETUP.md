...
> ✅ **你的站点已经用这个方式上线了，不用重新做**：
> - 网址：<https://peppy-starship-0a036b.netlify.app>
> - 站点名：`peppy-starship-0a036b`　Site ID：`52b4591d-245e-48ce-aa90-cf2a8ede0823`（已认领）
> - 日常更新**不用再拖文件夹**，直接用 6.4 的**方式②一条命令**即可（网址保持不变）。
...
4. **确认站点是「公开访问」**（⭐ 最容易漏、后果最严重）
   ...
   自检：用**无痕窗口**打开线上网址（或手机流量 / 另一台设备），能看到登录页才算通过。
   > 已处理：本站点的 `sso_login` 已关闭。注意 Netlify 的**账号级** `site_sso_login` 对新建站点默认仍可能是开启的，每次新建站点都要回头检查这一项。
...
| **② Netlify CLI**（✅ 已配好，见下方） | 在项目目录跑一条命令：`powershell -ExecutionPolicy Bypass -File .\update-online.ps1` | 不变 | 想要"一条命令"搞定，又不碰 GitHub |
...
#### 方式②（Netlify CLI）已经配好了 —— 以后用这一条命令

一次性配置**已完成**：`netlify-cli` 装在项目依赖里、CLI 已登录（`niao811@qq.com`）、项目已关联到站点 `peppy-starship-0a036b`（关联记录写在 `.netlify/state.json`，已被 gitignore，不进 git）。

**以后每次改完代码，在项目目录跑这一条就够了**：
...
任一环节失败会**立刻中止**，线上仍是上一个正常版本、不会变坏；成功会打印 `Production URL: https://peppy-starship-0a036b.netlify.app`（**网址不变**）。
实测：构建约 5 秒、上传 + 上线约 20 秒。

> 💡 **脚本必须保存为 `UTF-8 with BOM`**：脚本里的中文在 Windows PowerShell 5.1 下若没有 BOM 会被按 ANSI 解码，多字节序列会吃掉后面的引号/大括号，直接报 `缺少右"}"` 之类的语法错误（内容看起来完全正常，所以很难自己发现）。用 VS Code 右下角编码菜单选 **UTF-8 with BOM** 再保存。

> 💡 **为什么加 `--no-build`**：不加的话 Netlify 侧会**再跑一遍 `npm run build`**（因为站点面板里配了构建命令），白白多花时间；我们本地已经构建好，直接上传即可。

**换电脑 / 重装系统后怎么恢复**（3 条命令，网址依然不变）：

```powershell
npm install                                                      # 装回 netlify-cli
npx netlify-cli login                                            # 浏览器里点 Authorize
npx netlify-cli link --id 52b4591d-245e-48ce-aa90-cf2a8ede0823   # 关联回原站点
```

查状态：`npx netlify-cli status`，应显示 `Current project: peppy-starship-0a036b` 与 `Project URL: https://peppy-starship-0a036b.netlify.app`。
