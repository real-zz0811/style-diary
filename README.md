...
| 2 | 想给朋友看自己的搭配 | 只能用自己的设备，`localhost:5173` **不是网址** |
| 3 | 好不容易拼好的一套搭配 | 只能自己看，没有可交付、可分享的**产物** |

于是产品目标被压成三句话：

> **① 数据跟着账号走，不跟着设备走　② 有一个随时能打开的固定网址　③ 图片容量足够**
...
| **灵感墙** | 双列瀑布流（交替长宽比）+ 文字备注 | 更好地起到审美积累作用 |
| **账号** | 邮箱密码注册 / 登录，会话持久（关浏览器仍在线） | 开放注册：拿到网址的人自助开通，**账号之间数据硬隔离** |
| **删除** | 手机**长按 0.55s** / 电脑**右键** → 确认框 → 删记录 + 连带清理云端图片 | 删搭配**只删自己的长图**；单品照片属于单品记录，不受影响 |
